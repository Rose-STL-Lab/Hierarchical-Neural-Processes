...
class Supervisor:
    def __init__(self, random_seed, **kwargs):
        ...
        self.l1_z_mu_all = torch.zeros(self.z_dim, self.zw_dim,self.zw_dim).to(device)
        self.l1_z_cov_all = torch.ones(self.z_dim, self.zw_dim,self.zw_dim).to(device)
        self.l2_z_mu_all = None
        self.l2_z_cov_all = None
        self.l1_z_mu_c = torch.zeros(self.z_dim, self.zw_dim,self.zw_dim).to(device)
        self.l1_z_cov_c = torch.ones(self.z_dim, self.zw_dim,self.zw_dim).to(device)
        self.l2_z_mu_c = None
        self.l2_z_cov_c = None

        self.l2_z_mu_all_test = None
        self.l2_z_cov_all_test = None
    ...
    def save_model(self, epoch, l1_z_total, l2_z_total, test_l2_z_total, outputs, saved_model, val_nll, nll, mae):
        if not os.path.exists('weights'):
            os.makedirs('weights')
        if not os.path.exists('mc_mfhnp_results'):
            os.makedirs('mc_mfhnp_results')
        ...
        torch.save(test_l2_z_total, 'weights/test_l2_z_seed%d_epo%d.tar' % (self.random_seed, epoch))
        np.savez_compressed('mc_mfhnp_results/test_seed%d.npz'% (self.random_seed), **outputs)
        self._logger.info("Saved model at {}".format(epoch))

        with open(r'mc_mfhnp_results/metric_seed%d.csv'% (self.random_seed), 'a') as f:
            writer = csv.writer(f)
    ...
    def load_model(self):
        ...
        checkpoint3 = torch.load('weights/test_l2_z_seed%d_epo%d.tar' % (self.random_seed, self._epoch_num), map_location='cpu')
        ...
        self.l2_z_mu_all_test = checkpoint3[0].to(device)
        self.l2_z_cov_all_test = checkpoint3[1].to(device)
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                x, y = self._test_prepare_data(x, y)
                _, _, output_mu, output_cov = self.model(l2_x_all=x, test=True, l2_z_mu_all= self.l2_z_mu_all_test, l2_z_cov_all=self.l2_z_cov_all_test)
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for index, ((l1_x, l1_y), (l2_x, l2_y)) in enumerate(zip(l1_train_iterator, l2_train_iterator)): # need to be fixed
                ...
                l1_output_mu, l1_output_cov, l2_output_mu, l2_output_cov, l1_truth, l2_truth, self.l1_z_mu_all, self.l1_z_cov_all, self.l1_z_mu_c, self.l1_z_cov_c, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c, self.l2_z_mu_all_test, self.l2_z_cov_all_test = self.model(l1_x, l1_y, l2_x, l2_y, batches_seen, False, self.l1_z_mu_all, self.l1_z_cov_all, self.l1_z_mu_c, self.l1_z_cov_c, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c)
                ...
                self.l2_z_mu_all_test = self.l2_z_mu_all_test.detach()
                self.l2_z_cov_all_test = self.l2_z_cov_all_test.detach()
            ...
            if val_loss < min_val_loss:
                ...
                saved_test_l2_z_total = torch.stack([self.l2_z_mu_all_test, self.l2_z_cov_all_test], dim=0)

            elif val_loss >= min_val_loss:
                wait += 1
                if wait == patience:
                    model_file_name = self.save_model(saved_epoch, saved_l1_z_total, saved_l2_z_total, saved_test_l2_z_total, saved_outputs, saved_model, saved_val_nll, saved_nll, saved_mae)
    ...
    def _test_prepare_data(self, x, y):
