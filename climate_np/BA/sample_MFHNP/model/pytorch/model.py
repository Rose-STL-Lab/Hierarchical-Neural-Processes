...
class Model(nn.Module):
    ...
    def ancestral_sample_z(self, mean, var):
        """Reparameterisation trick."""
        eps = torch.autograd.Variable(var.data.new(var.size(0),var.size(1),var.size(2),var.size(3)).normal_()).to(device)
    ...
    def xy_to_r(self, x, y, level):
        if level == 1:
            r_mu, r_cov = self.l1_encoder_model(torch.cat([x, y],dim=-3))
        elif level == 2:
            r_mu, r_cov = self.l2_encoder_model(torch.cat([x, y],dim=-3))
    ...
    def forward(self, l1_x_all=None, l1_y_all=None, l2_x_all=None, l2_y_all=None, batches_seen=None, test=False, l1_z_mu_all=None, l1_z_cov_all=None, l1_z_mu_c=None, l1_z_cov_c=None, l2_z_mu_all=None, l2_z_cov_all=None, l2_z_mu_c=None, l2_z_cov_c=None):
        ...
        if test==False:
            ...
            l1_r_mu_c, l1_r_cov_c = self.xy_to_r(l1_x_c, l1_y_c, level=1)
            ...
            l1_zs_for_l2_test = self.sample_z(l1_z_mu_all, l1_z_cov_all, 50)
            ...
            #l2_encoder
            l2_r_mu_all, l2_r_cov_all = self.xy_to_r(l2_x_all, l2_y_all, level=2)

            l2_r_mu_all_test = torch.repeat_interleave(torch.unsqueeze(l2_r_mu_all,dim=1),50,dim=1)
            l2_r_cov_all_test = torch.repeat_interleave(torch.unsqueeze(l2_r_cov_all,dim=1),50,dim=1)
            ...
            l2_r_mu_c, l2_r_cov_c = self.xy_to_r(l2_x_c, l2_y_c, level=2)
