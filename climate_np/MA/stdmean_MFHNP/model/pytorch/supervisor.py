...
class Supervisor:
    ...
    def save_model(self, epoch, l1_z_total, l2_z_total, outputs, saved_model, val_nll, nll, mae):
        if not os.path.exists('weights'):
            os.makedirs('weights')
        if not os.path.exists('stdmean_mfhnp_results'):
            os.makedirs('stdmean_mfhnp_results')
        ...
        np.savez_compressed('stdmean_mfhnp_results/test_seed%d.npz'% (self.random_seed), **outputs)
        self._logger.info("Saved model at {}".format(epoch))

        with open(r'stdmean_mfhnp_results/metric_seed%d.csv'% (self.random_seed), 'a') as f:
            writer = csv.writer(f)
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                ...
                _, _, output_mu, output_cov = self.model(l2_x_all=x, test=True, l2_z_mu_all= self.l2_z_mu_all, l2_z_cov_all=self.l2_z_cov_all)
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            if val_loss < min_val_loss:
                ...
                wait += 1
                if wait == patience:
                    model_file_name = self.save_model(saved_epoch, saved_l1_z_total, saved_l2_z_total, saved_outputs, saved_model, saved_val_nll, saved_nll, saved_mae)
