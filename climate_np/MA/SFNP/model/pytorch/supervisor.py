...
class Supervisor:
    ...
    def save_model(self, epoch, l2_z_total, outputs, saved_model, val_nll, nll, mae):
        if not os.path.exists('weights'):
            os.makedirs('weights')
        if not os.path.exists('sfnp_results'):
            os.makedirs('sfnp_results')
        ...
        np.savez_compressed('sfnp_results/test_seed%d.npz'% (self.random_seed), **outputs)
        self._logger.info("Saved model at {}".format(epoch))

        with open(r'sfnp_results/metric_seed%d.csv'% (self.random_seed), 'a') as f:
            writer = csv.writer(f)
            writer.writerow([nll, mae])

        return 'seed%d/model_epo%d.tar' % (self.random_seed, epoch)
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                ...
                output_mu, output_cov = self.model(l2_x_all=x, test=True, l2_z_mu_all= self.l2_z_mu_all, l2_z_cov_all=self.l2_z_cov_all)
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for index, (l2_x, l2_y) in enumerate(l2_train_iterator): # need to be fixed
                optimizer.zero_grad()

                l2_x, l2_y = self._train_l2_prepare_data(l2_x, l2_y)


                l2_output_mu, l2_output_cov, l2_truth, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c = self.model(l2_x, l2_y, batches_seen, False, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c)
