...
class Supervisor:
    def __init__(self, random_seed, **kwargs):
        ...
        self.zw_dim = int(self._model_kwargs.get('zw_dim',14))
        ...
        self.l2_z_mu_all = torch.zeros(self.z_dim, self.zw_dim,self.zw_dim).to(device)
        self.l2_z_cov_all = torch.ones(self.z_dim, self.zw_dim,self.zw_dim).to(device)
        self.l2_z_mu_c = torch.zeros(self.z_dim, self.zw_dim,self.zw_dim).to(device)
        self.l2_z_cov_c = torch.ones(self.z_dim, self.zw_dim,self.zw_dim).to(device)
    ...
    def save_model(self, epoch, l2_z_total, outputs, saved_model, val_nll, nll, mae):
        if not os.path.exists('weights'):
            os.makedirs('weights')
        if not os.path.exists('mfnp_results'):
            os.makedirs('mfnp_results')
        ...
        np.savez_compressed('mfnp_results/test_seed%d.npz'% (self.random_seed), **outputs)
        self._logger.info("Saved model at {}".format(epoch))

        with open(r'mfnp_results/metric_seed%d.csv'% (self.random_seed), 'a') as f:
            writer = csv.writer(f)
            writer.writerow([nll, mae])
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (y_low, x, y) in enumerate(val_iterator):
                y_low, x, y = self._test_prepare_data(y_low, x, y)
                output_mu, output_cov = self.model(l1_y_all=y_low, l2_x_all=x, test=True, l2_z_mu_all= self.l2_z_mu_all, l2_z_cov_all=self.l2_z_cov_all)
            ...
            # y_preds_mu = y_preds_mu[:52*30] #15*30
            # y_preds_cov = y_preds_cov[:52*30]
            # y_truths = y_truths[:52*30]
            ...
            nll_metric, mae_metric = self._test_loss(y_preds_mu, y_preds_cov, y_truths)
            # self._writer.add_scalar('{} loss'.format(dataset), nll_metric, batches_seen) #check

            return nll_metric, mae_metric, {'pred_mu': y_preds_mu, 'pred_cov': y_preds_cov, 'truth': y_truths}
    ...
    def _train(self, base_lr,
        ...
        # check data size
        l2_data_size = self._data['train_loader'].size
        print("l2_data_size:",l2_data_size)
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for index, (l1_y, l2_x, l2_y) in enumerate(train_iterator): # need to be fixed
                optimizer.zero_grad()

                l1_y, l2_x, l2_y = self._train_l2_prepare_data(l1_y, l2_x, l2_y)


                l2_output_mu, l2_output_cov, l2_truth, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c = self.model(l1_y, l2_x, l2_y, batches_seen, False, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c)
                ...
                l2_nll_loss,l2_kld_loss = self._compute_loss(l2_output_mu, l2_output_cov, l2_truth, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c)
                loss = l2_nll_loss + l2_kld_loss
            ...
            _, val_loss, _ = self.evaluate(dataset='val', batches_seen=batches_seen)
            end_time = time.time()
            ...
            if (epoch_num % log_every) == log_every - 1:
                message = 'Epoch [{}/{}] ({}) train_loss: {:.4f}, l2_nll: {:.4f}, l2_kld: {:.4f}, val_mae: {:.4f}, lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           np.mean(losses), np.mean(l2_nll_losses), np.mean(l2_kld_losses), val_loss, lr_scheduler.get_lr()[0],
                                           (end_time - start_time))
                self._logger.info(message)

            if (epoch_num % test_every_n_epochs) == test_every_n_epochs - 1:
                test_nll_loss, test_mae_loss, test_outputs = self.evaluate(dataset='test', batches_seen=batches_seen)
                message = 'Epoch [{}/{}] ({}) test_nll: {:.4f}, test_mae: {:.4f}, lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           test_nll_loss, test_mae_loss, lr_scheduler.get_lr()[0],
                                           (end_time - start_time))
                self._logger.info(message)
            ...
            if val_loss < min_val_loss:
                ...
                wait += 1
                if wait == patience:
                    model_file_name = self.save_model(saved_epoch, saved_l2_z_total, saved_outputs, saved_model, saved_val_nll, saved_nll, saved_mae)

                    self._logger.info(
                        'Final Val loss {:.4f}, Test NLL loss {:.4f}, Test MAE loss {:.4f},'
                        'saving to {}'.format(saved_val_nll, saved_nll, saved_mae, model_file_name))
    ...
    def _test_prepare_data(self, y_low, x, y):

        y_low = torch.from_numpy(y_low).float()
        x = torch.from_numpy(x).float()
        y = torch.from_numpy(y).float()

        return y_low.to(device), x.to(device), y.to(device)

    def _train_l2_prepare_data(self, y_low, x, y):
    ...
    def _compute_loss(self, l2_output_mu, l2_output_cov, l2_truth, l2_z_mu_all, l2_z_cov_all, l2_z_mu_c, l2_z_cov_c):
        ...
        return nll_loss(l2_output_mu, l2_output_cov, l2_truth), kld_gaussian_loss(l2_z_mu_all, l2_z_cov_all, l2_z_mu_c, l2_z_cov_c)

    def _test_loss(self, y_predicted_mu, y_predicted_cov, y_truth):

        return nll_metric(y_predicted_mu, y_predicted_cov, y_truth), mae_metric(y_predicted_mu, y_truth)
