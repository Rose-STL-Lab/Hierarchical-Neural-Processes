...
class Supervisor:
    ...
    def save_model(self, epoch, l1_z_total, l2_z_total, test_l2_z_total, outputs, saved_model, val_nll, nll, mae, non_mae):
        ...
        with open(r'mc_mfhnp_results/metric_seed%d.csv'% (self.random_seed), 'a') as f:
            writer = csv.writer(f)
            writer.writerow([val_nll, nll, mae, non_mae])
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y, adj_mx) in enumerate(val_iterator):
                ...
                _, _, output_mu, output_cov = self.model(l2_x_all=x, l2_adj_all=adj_mx, test=True, l2_z_mu_all= self.l2_z_mu_all_test, l2_z_cov_all=self.l2_z_cov_all_test)
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for index, ((l1_x, l1_y, l1_adj), (l2_x, l2_y, l2_adj)) in enumerate(zip(l1_train_iterator, l2_train_iterator)): # need to be fixed
                ...
                l1_output_mu, l1_output_cov, l2_output_mu, l2_output_cov, l1_truth, l2_truth, self.l1_z_mu_all, self.l1_z_cov_all, self.l1_z_mu_c, self.l1_z_cov_c, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c, self.l2_z_mu_all_test, self.l2_z_cov_all_test = self.model(l1_x, l1_y, l1_adj, l2_x, l2_y, l2_adj, batches_seen, False, self.l1_z_mu_all, self.l1_z_cov_all, self.l1_z_mu_c, self.l1_z_cov_c)
            ...
            if val_loss < min_val_loss:
                ...
                wait += 1
                if wait == patience:
                    model_file_name = self.save_model(saved_epoch, saved_l1_z_total, saved_l2_z_total, saved_test_l2_z_total, saved_outputs, saved_model, saved_val_nll, saved_nll, saved_mae, saved_non_mae)
