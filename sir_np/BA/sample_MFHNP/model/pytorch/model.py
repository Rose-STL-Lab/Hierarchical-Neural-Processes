...
class Model(nn.Module):
    ...
    def xy_to_r(self, x, y, adj, level):
        if level == 1:
            r_mu, r_cov = self.l1_encoder_model(torch.cat([x, y],dim=-1), adj)
        elif level == 2:
            r_mu, r_cov = self.l2_encoder_model(torch.cat([x, y],dim=-1), adj)
    ...
    def forward(self, l1_x_all=None, l1_y_all=None, l1_adj_all=None, l2_x_all=None, l2_y_all=None, l2_adj_all=None, batches_seen=None, test=False, l1_z_mu_all=None, l1_z_cov_all=None, l1_z_mu_c=None, l1_z_cov_c=None, l2_z_mu_all=None, l2_z_cov_all=None, l2_z_mu_c=None, l2_z_cov_c=None):
        ...
        if test==False:
            ...
            #l1_encoder
            l1_r_mu_all, l1_r_cov_all = self.xy_to_r(l1_x_all, l1_y_all, l1_adj_all, level=1)
            l1_r_mu_c, l1_r_cov_c = self.xy_to_r(l1_x_c, l1_y_c, l1_adj_c, level=1)
            l1_z_mu_all, l1_z_cov_all = self.ba_z_agg(l1_r_mu_all,l1_r_cov_all,l1_z_mu_all,l1_z_cov_all)
            l1_z_mu_c, l1_z_cov_c = self.ba_z_agg(l1_r_mu_c,l1_r_cov_c,l1_z_mu_all,l1_z_cov_all)

            #ancestral sampling z at level 1
            l1_zs_for_l2 = self.sample_z(l1_z_mu_all, l1_z_cov_all, l2_x_t.size(0))
            l1_zs_for_l2_test = self.sample_z(l1_z_mu_all, l1_z_cov_all, 780)

            # NN between two fidelity levels
            l2_z_mu_all_0,l2_z_cov_all_0 = self.z2_z1_agg(l1_zs_for_l2)

            l2_z_mu_all_0_test,l2_z_cov_all_0_test = self.z2_z1_agg(l1_zs_for_l2_test)

            #l2_encoder
            l2_r_mu_all, l2_r_cov_all = self.xy_to_r(l2_x_all, l2_y_all, l2_adj_all, level=2)

            l2_r_mu_all_test = torch.repeat_interleave(torch.unsqueeze(l2_r_mu_all,dim=1),780,dim=1)
            l2_r_cov_all_test = torch.repeat_interleave(torch.unsqueeze(l2_r_cov_all,dim=1),780,dim=1)

            l2_r_mu_all = torch.repeat_interleave(torch.unsqueeze(l2_r_mu_all,dim=1),l2_x_t.size(0),dim=1)
            l2_r_cov_all = torch.repeat_interleave(torch.unsqueeze(l2_r_cov_all,dim=1),l2_x_t.size(0),dim=1)


            l2_r_mu_c, l2_r_cov_c = self.xy_to_r(l2_x_c, l2_y_c, l2_adj_c, level=2)
            l2_r_mu_c = torch.repeat_interleave(torch.unsqueeze(l2_r_mu_c,dim=1),l2_x_t.size(0),dim=1)
            l2_r_cov_c = torch.repeat_interleave(torch.unsqueeze(l2_r_cov_c,dim=1),l2_x_t.size(0),dim=1)

            l2_z_mu_all, l2_z_cov_all = self.ba_z_agg(l2_r_mu_all,l2_r_cov_all,l2_z_mu_all_0,l2_z_cov_all_0)
            l2_z_mu_c, l2_z_cov_c = self.ba_z_agg(l2_r_mu_c,l2_r_cov_c,l2_z_mu_all_0,l2_z_cov_all_0)

            l2_z_mu_all_test, l2_z_cov_all_test = self.ba_z_agg(l2_r_mu_all_test,l2_r_cov_all_test,l2_z_mu_all_0_test,l2_z_cov_all_0_test)

            #sample z at level 2
            l1_zs = self.sample_z(l1_z_mu_all, l1_z_cov_all, l1_x_t.size(0))
            l2_zs = self.ancestral_sample_z(l2_z_mu_all, l2_z_cov_all)
            ...
            return l1_output_mu, l1_output_cov, l2_output_mu, l2_output_cov, l1_truth, l2_truth, l1_z_mu_all, l1_z_cov_all, l1_z_mu_c, l1_z_cov_c, l2_z_mu_all, l2_z_cov_all, l2_z_mu_c, l2_z_cov_c, l2_z_mu_all_test, l2_z_cov_all_test
