...
class Supervisor:
    ...
    def save_model(self, epoch, l2_z_total, outputs, saved_model, val_nll, nll, mae, non_mae):
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y, adj_mx) in enumerate(val_iterator):
                ...
                output_mu, output_cov = self.model(l2_x_all=x, l2_adj_all=adj_mx, test=True, l2_z_mu_all= self.l2_z_mu_all, l2_z_cov_all=self.l2_z_cov_all)
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for index, (l2_x, l2_y, l2_adj) in enumerate(l2_train_iterator): # need to be fixed
                optimizer.zero_grad()

                l2_x, l2_y, l2_adj = self._train_l2_prepare_data(l2_x, l2_y, l2_adj)


                l2_output_mu, l2_output_cov, l2_truth, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c = self.model(l2_x, l2_y, l2_adj, batches_seen, False, self.l2_z_mu_all, self.l2_z_cov_all, self.l2_z_mu_c, self.l2_z_cov_c)
            ...
            if val_loss < min_val_loss:
                ...
                wait += 1
                if wait == patience:
                    model_file_name = self.save_model(saved_epoch, saved_l2_z_total, saved_outputs, saved_model, saved_val_nll, saved_nll, saved_mae, saved_non_mae)
